...
newtype DateTimeSpan
  = DateTimeSpan (DateTime, DateTime)
...
crtDtTmSpan strdtStart strdtEnd separator =
  let maybeDtstart = (readMaybe :: String -> Maybe Int) <$> splitOn separator strdtStart
  ...
  where
    checkDateAttrRanges (y, mo, d, h, mi, s)
      | mo > 12 || mo < 1 = False
      | d > 31 || d < 1 = False
      | h > 23 || h < 0 = False
      | mi > 59 || mi < 0 = False
